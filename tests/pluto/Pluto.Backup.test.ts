...
import { Apollo, JWTVerifiableCredentialRecoveryId } from "../../src";
...
describe("Pluto", () => {
  ...
  describe("Backup", () => {
    ...
    test("dids + did_pairs", async () => {
      ...
      expect(result.dids[0]).to.have.property("did", Fixtures.Backup.hostDID.toString());
      expect(result.dids[0]).to.have.property("alias", undefined);
      expect(result.dids[1]).to.have.property("did", Fixtures.Backup.targetDID.toString());
